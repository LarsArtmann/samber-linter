...
| a.11 | Full test suite green                                 | `GOEXPERIMENT=jsonv2 CGO_ENABLED=0 go test ./... -count=1` → ok (driver 2.3s, healthaudit, healthwash, plugin 50.7s); `go vet` clean; final `nix flake check` on the fixed tree → **"all checks passed!"** (build + full tests + hermetic lint + treefmt + dprint)                                                                                                                                                                                                                                                                                                                                                  |
...
4. **First draft of `scripts/ecology-scan.sh` had three real bugs**: (a) it
   ...
   _packages_ fail to load don't exit 2 under `--check` — they report
