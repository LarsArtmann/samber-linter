...
| HW-8          | pending next scan  | none known — the shape requires a NAMED result to compile (an unnamed `error` result with an empty body is a compile error), which makes it a deliberate authoring act | **0** — a check body with zero statements implicitly returns nil; the check cannot fail. The named-result requirement means the author opted into the no-op, not stumbled into it.                               | reasoned suppression, or implement the real check                        |
...
- **HW-8 boundaries:** zero-statement bodies are its whole predicate; a
  `panic(...)` body is negative (it is loud, not silently green) and a naked
  `return` among other statements needs flow data. HW-7 and HW-8 are mutually
