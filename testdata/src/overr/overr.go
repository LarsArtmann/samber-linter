// Package overr exercises the Override* registration family (di.go:187-251):
// identical wrappers, identical health semantics, so HW-1 must fire exactly
// as it does for Provide*. Aliases (As) are tracked but never attributed.
package overr
...
var _ = func() bool {
	do.Override(nil, NewResource) // want `HW-1: .*`
	return true
}()

var _ = func() bool {
	do.OverrideNamed(nil, "resource-2", NewResource) // want `HW-1: .*`
	return true
}()

var _ = func() bool {
	do.OverrideValue(nil, Resource{}) // want `HW-1: .*`
	return true
}()
